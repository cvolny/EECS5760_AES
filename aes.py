...
#<editor-fold desc="sbox and isbox lookup tables">
__sbox = [
...
#<editor-fold desc="cmix and icmix lookup tables">
__cmix_02= [
...
#<editor-fold desc="rcon lookup table">
__rcon = [
...
def str_to_state(v):
    """Convert 16-byte block to 4x4 byte state matrix."""
    assert len(v) == 16
...
def state_to_str(b):
    """Convert 4x4 byte state matrix to 16-byte block."""
...
def __cmix_v(v):
    """Performs column mix on column-vector v. Modifies v in place."""
    t = v[:]
    v[0] = __cmix_02[t[0]] ^ __cmix_03[t[1]] ^ t[2]             ^ t[3]
    v[1] = t[0]            ^ __cmix_02[t[1]] ^ __cmix_03[t[2]]  ^ t[3]
    v[2] = t[0]            ^ t[1]            ^ __cmix_02[t[2]]  ^ __cmix_03[t[3]]
    v[3] = __cmix_03[t[0]] ^ t[1]            ^ t[2]             ^ __cmix_02[t[3]]
...
def __icmix_v(v):
    """Inversion of __cmix_v(v). Modifies v in place."""
    t = v[:]
    v[0] = __cmix_14[t[0]] ^ __cmix_11[t[1]] ^ __cmix_13[t[2]] ^ __cmix_09[t[3]]
    v[1] = __cmix_09[t[0]] ^ __cmix_14[t[1]] ^ __cmix_11[t[2]] ^ __cmix_13[t[3]]
    v[2] = __cmix_13[t[0]] ^ __cmix_09[t[1]] ^ __cmix_14[t[2]] ^ __cmix_11[t[3]]
    v[3] = __cmix_11[t[0]] ^ __cmix_13[t[1]] ^ __cmix_09[t[2]] ^ __cmix_14[t[3]]
...
def rdblk(filename, size=16, padded=True):
    """Read size bytes from filename; yielding byte by byte with random padding data if specified."""
...
def ntoken(v, n):
    """Return n sized tokens from v."""
    for i in xrange(0, len(v), n):
        yield v[i:i + n]
